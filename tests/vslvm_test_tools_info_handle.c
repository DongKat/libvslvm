...
#include "../vslvmtools/info_handle.h"

/* Tests the info_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int vslvm_test_tools_info_handle_initialize(
     void )
{
 ...
 * Returns 1 if successful or 0 if not
 */
int vslvm_test_tools_info_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	VSLVM_TEST_RUN(
	 ...
	 vslvm_test_tools_info_handle_initialize );

	VSLVM_TEST_RUN(
	 "info_handle_free",
	 vslvm_test_tools_info_handle_free );

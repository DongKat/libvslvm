...
void (*vslvmtools_signal_signal_handler)( vslvmtools_signal_t ) = NULL;
...
{
	static char *function = "vslvmtools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( vslvmtools_signal_signal_handler != NULL )
			{
				vslvmtools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "vslvmtools_signal_attach";
	...
	vslvmtools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) vslvmtools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	vslvmtools_signal_initialize_memory_debug();
#endif
...
int vslvmtools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "vslvmtools_signal_detach";
	...
	vslvmtools_signal_signal_handler = NULL;
